...
{ 
  ...
  public:
#ifdef LAYERS
  union  { struct { float x; float y; float wsum; int gen; }; struct { float u; float bottom[LAYERS-1]; }; } o;
#else
  union  { struct { float x; float y; float wsum; int gen; }; struct { double u; }; } o;
#endif
  float  ilam;
  float  jlam;
#endif
...
//  vector<Point>  neigh = getNearestNeighbors(p);
    ...
    for ( int i = 0; i < 4; ++i ) 
      if ( getNode(neigh[i])->m )
        ...
        {
#ifdef DEFORM
          dh[i] *= 0.25*det(relVect(getNode(neigh[i]),pn),
                            relVect(getNode(neigh[i+1]),getNode(neigh[(i+3)&3]))
                           +relVect(getNode(neigh[(i+5)&7]),getNode(neigh[i+4])))/pn->distSquare(getNode(neigh[i]));
#endif
          dhsum += dh[i];
        }
    dhsum *= dt*odiff;
    ...
    for ( int i = 1; i < m-1; ++i )
      for ( int j = 1; j < n-1; ++j )
      {
        ...
        if ( pn->b&2 )
        {
          ...
          {
            double  f = 0.25*det(relVect(getNode(i+1,j),getNode(i,j)),
                                 relVect(getNode(i,j+1),getNode(i,j-1))
                                +relVect(getNode(i+1,j+1),getNode(i+1,j-1)))/pn->distSquare(getNode(i+1,j));
//            printf ( "%i %i %e\n", i, j, f );
          ...
          {
            double  f = 0.25*det(relVect(getNode(i,j+1),getNode(i,j)),
                                 relVect(getNode(i-1,j),getNode(i+1,j))
                                +relVect(getNode(i-1,j+1),getNode(i+1,j+1)))/pn->distSquare(getNode(i,j+1));
//            printf ( "%i %i %e\n", i, j, f );
  ...
  void limit ( vector<int>& v )
  {
    vector<int>  cml = v, cmr = v;
    for ( int i = 1; i < v.size(); ++i ) 
      if ( cml[i-1] > cml[i] )  cml[i] = cml[i-1];
    for ( int i = v.size()-2; i >= 0; --i ) 
      if ( cmr[i+1] > cmr[i] )  cmr[i] = cmr[i+1];
    for ( int i = 1; i < v.size()-1; ++i ) 
      if ( v[i] <= cml[i-1] && v[i] <= cmr[i+1] )  v[i] = 0;
  }

  int isRight ( Point o, Point p, Point q )
  {
    return  (p.i-o.i)*(q.j-o.j) > (q.i-o.i)*(p.j-o.j);
  } 

  vector<Point> shorten ( vector<Point> &p )
  {
    vector<Point>  q;
    q.push_back(p[0]);
    for ( int i = 1; i < p.size()-1; ++i )
      if ( isRight(p[i-1],p[i],p[i+1]) )  q.push_back(p[i]);
    q.push_back(p.back());
    return  q;
  }

  vector<Point> mark_Line ( Point p, Point q, vector<Point> rim )
  {
    vector<Point>  pts, neigh;
    if ( q.i-p.i >= abs(q.j-p.j) )
      for ( int i = p.i; i <= q.i; ++i )
        pts.push_back(Point(i,p.j+round(double(i-p.i)/(q.i-p.i)*(q.j-p.j)))); 
    else if ( p.i-q.i >= abs(q.j-p.j) )
      for ( int i = p.i; i >= q.i; --i )
        pts.push_back(Point(i,p.j+round(double(i-p.i)/(q.i-p.i)*(q.j-p.j)))); 
    else if ( q.j-p.j >= abs(q.i-p.i) )
      for ( int j = p.j; j <= q.j; ++j )
        pts.push_back(Point(p.i+round(double(j-p.j)/(q.j-p.j)*(q.i-p.i)),j));
    else if ( p.j-q.j >= abs(q.i-p.i) )
      for ( int j = p.j; j >= q.j; --j )
        pts.push_back(Point(p.i+round(double(j-p.j)/(q.j-p.j)*(q.i-p.i)),j));
...
//      printf ( "L %i %i\n", pts[i].i, pts[i].j );
      ...
      for ( vector<Point>::iterator u = neigh.begin(); u != neigh.end(); ++u )
      {
        if ( !(getNode(u)->m&32) && isRight(p,*u,q) )
        {
          getNode(u)->m |= 32;
          rim.push_back(*u);
        }
  ...
  void equilibrate ( double dt )
  {
    vector<Point>  pts, rim, neigh;
    OceanNode  *pn, *pu;
    for ( int i = 0; i < m; ++i )
      for ( int j = 0; j < n; ++j )  getNode(i,j)->m = getNode(i,j)->o.gen = 0;

// Find the convex hull around all continent areas
    vector<int>  jmin(m), jmax(m);
    for ( int i = 0; i < m; ++i )
    {
      for ( jmin[i] = 0; jmin[i] < n && getNode(i,jmin[i])->b; ++jmin[i] );
      jmin[i] = -jmin[i];
      for ( jmax[i] = n-1; jmax[i] >= 0 && getNode(i,jmax[i])->b; --jmax[i] );
    }
    limit(jmin);
    limit(jmax);
    for ( int i = 1; i < m-1; ++i )
      if ( jmin[i] < 0 )  pts.push_back(Point(i,-jmin[i]));
    for ( int i = m-2; i >= 1; --i )
      if ( jmax[i] > 0 )  pts.push_back(Point(i,jmax[i]));
    pts.push_back(pts[0]);
    ...
    for ( int i = 0; i < pts.size(); ++i )
      if ( i < pts.size()-1 )  rim = mark_Line ( pts[i], pts[i+1], rim );

// Compute the mean rotation angle
    int     k = 0;
    ...
    double  alpha = atan2((yi*k-ysum*isum)-(xj*k-xsum*jsum),
                          (xi*k-xsum*isum)+(yj*k-ysum*jsum));    
//    printf("%e\n",alpha);
    double  c = cos(alpha);
    double  s = sin(alpha);

// Rotate the entire domain oppositve to the backbone
    for ( int i = 0; i < m; ++i )
      for ( int j = 0; j < n; ++j )
      {
        pn = getNode(i,j);
        pn->o.x = c*pn->x+s*pn->y;
        pn->o.y = -s*pn->x+c*pn->y;
        pn->x = pn->o.x;
        pn->y = pn->o.y;
      }

// Find the nodes adjacent to the convex hull
    pts.clear();
    for ( int i = 0; i < rim.size(); ++i )
      if ( (pn=getNode(rim[i]))->m&32 )
      {
        pn->m &= 31;
        neigh = getNeighbors(rim[i]);
        for ( vector<Point>::iterator u = neigh.begin(); u != neigh.end(); ++u )
          if ( !((pu=getNode(u))->m&16) )
          {
            if ( ++pu->m == 1 )  pu->x = pu->y = pu->o.wsum = 0;
            double  w = 1/sqrt((pu->o.x-pn->x)*(pu->o.x-pn->x)+(pu->o.y-pn->y)*(pu->o.y-pn->y));
//          if ( w < 1 )  w = 1;
            pu->o.wsum += w;
            pu->x += w*(pn->x-rim[i].i); 
            pu->y += w*(pn->y-rim[i].j);
...
//    for ( int i = 0; i < m; ++i )
//      for ( int j = 0; j < n; ++j )
//        if ( getNode(i,j)->m && getNode(i,j)->m != 16 )
//          printf ( "%i %i %i %g\n", i, j, getNode(i,j)->m, getNode(i,j)->jlam );
//        getNode(i,j)->b = getNode(i,j)->m; 

//return;

// Iterate over all points except for the convex hull
    for ( int i = 0; i < pts.size(); ++i )
    {
      pn = getNode(pts[i]);
      pn->x /= pn->o.wsum;
      ...
      neigh = getNeighbors(pts[i]);
      for ( vector<Point>::iterator u = neigh.begin(); u != neigh.end(); ++u )
        if ( (pu=getNode(u))->m < 16 && pu->o.gen != pn->o.gen )
        {
          if ( !pu->m )  pu->x = pu->y = pu->o.wsum = 0;
          double  w = 1/sqrt((pu->o.x-pn->x-pts[i].i)*(pu->o.x-pn->x-pts[i].i)+(pu->o.y-pn->y-pts[i].j)*(pu->o.y-pn->y-pts[i].j));
//          if ( w < 1 )  w = 1;
          pu->o.wsum += w;
          pu->x += w*pn->x; 
          pu->y += w*pn->y; 
...
//    for ( int i = 0; i < m; ++i )
//      for ( int j = 0; j < n; ++j )
//        printf ( "%i\n", getNode(i,j)->b = getNode(i,j)->jlam ); 
//return;

// Find new position as a linear combination of the direct neighbors
    for ( int i = 0; i < pts.size(); ++i )
      if ( !((pn=getNode(pts[i]))->b&1) )
      { 
...
// Convert horizontal shift into an uplift rate and adjust layers
    for ( int i = 0; i < m; ++i )
      for ( int j = 0; j < n; ++j )
      {
        getNode(i,j)->m = 0;
    ...
    public:
      ...
      Connector ( OceanGrid *g, double hscale = 1, double x0 = 0, double y0 = 0, double alpha = 0 )
      {
        ...
        for ( int i = 0; i < g->m; ++i )
          for ( int j = 0; j < g->n; ++j )
            {
              if((i == 217 && j == 150) || (i == 10 && j == 10))
                std::cout <<i <<": openlem construct before x:y " << x[i][j] << ":" << y[i][j] << std::endl;
              x[i][j] = x0 + hscale*(c*g->getNode(i,j)->x-s*g->getNode(i,j)->y);
              y[i][j] = y0 + hscale*(s*g->getNode(i,j)->x+c*g->getNode(i,j)->y);
              if((i == 217 && j == 150) || (i == 10 && j == 10))
                std::cout <<i <<": openlem construct after x:y " << x[i][j] << ":" << y[i][j] << ", x0:y0 = " << x0 << ":" << y0 << std::endl;
            }
      }
      ...
      void update ( double dt , unsigned int aspect_timestep_number, unsigned int openlem_iteration)
      {
        ...
        for ( int i = 0; i < g->m; ++i )
          for ( int j = 0; j < g->n; ++j )
            {
              //if((i == 217 && j == 150) || (i == 10 && j == 10))
              //std::cout <<i <<": openlem update before x:y " << x[i][j] << ":" << y[i][j] << ", vx:vy = " << vx[i][j] << ":" << vy[i][j]<< std::endl;
              x[i][j] += vx[i][j]*dt;
              y[i][j] += vy[i][j]*dt;
              //if((i == 217 && j == 150) || (i == 10 && j == 10))
              //std::cout <<i <<": openlem update after x:y " << x[i][j] << ":" << y[i][j] << std::endl;
...
//    printf("%e\n",alpha);
        ...
        if(openlem_iteration == 0)
          g->write("openlem_output_ASP-t"+addZeroPaddedNumber(aspect_timestep_number,3) + "_OLM-i_"+addZeroPaddedNumber(openlem_iteration,3) + ".dat","H,L,X,Y,B,U,BOT");
            ...
            for (size_t i = 0; i <= x.size()-1; ++i)
              {
                ...
                assert(!isnan(y[i][j]));
                if((i == 217 && j == 150) || (i == 10 && j == 10))
                std::cout <<"openlem vtk x:y:h " << x[i][j] << ":" << y[i][j] << ":" << g->getNode(i,j)->h << std::endl;
